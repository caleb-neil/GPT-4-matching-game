...
def get_user_content(topic):
  ...
  file = f"prompts/{topic_dict[topic]}"
  with open(file, "r") as f:
    user_prompt = f.readline()
  with open('prompts/SystemPrompt.txt', "r") as s:
    system_prompt = s.readline()
  f.close
  s.close
  get_response(system_prompt, user_prompt)

get_user_content("calc") # Entering one of the keys from topic_dict will call GPT-4 with that specific prompt